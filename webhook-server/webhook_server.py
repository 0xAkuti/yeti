...
def generate_secure_webhook_id() -> uuid.UUID:
    """Generate a cryptographically secure webhook ID using HMAC"""
    # Get secret from environment or use default (in production, always use env var)
    secret = os.getenv('WEBHOOK_SECRET', 'default-secret-change-in-production').encode()
    
    # 4 bytes for timestamp
    timestamp = int(time.time()).to_bytes(4, 'big')
    
    # Generate 12-byte HMAC signature
    signature = hmac.new(secret, timestamp, hashlib.sha256).digest()[:12]
    
    # Combine timestamp + signature = 16 bytes total
    webhook_bytes = timestamp + signature
    
    # Convert to UUID format for compatibility
    return uuid.UUID(bytes=webhook_bytes)

def verify_webhook_id(webhook_id: str) -> bool:
    """Verify that a webhook ID was generated by our system"""
    try:
        # Convert UUID string back to bytes
...
class WebhookManager:
    def create_webhook(self) -> dict:
        webhook_uuid = generate_secure_webhook_id()
    ...
    def webhook_exists(self, webhook_id: str) -> bool:
        return verify_webhook_id(webhook_id)
